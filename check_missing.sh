...
if [ $NUMBER_F = 0 ]; then
    echo "[06:03AM]: No files in the data directory." >> /home/jline/t1b.txt
    ...
    do
        ...
        if [ $N -lt $NUMBER_TILES ]; then 
            TIME=$(basename $d);
            for i in "S33XX" "S33YY" "S34XX" "S34YY" "S35XX" "S35YY" "S36XX" "S36YY"; do 
                
                if [ ! -f $d/$i* ]; then
                    echo `date +'[%H:%M%p]'`": $i"_"$TIME".txt" file not found" >> /home/jline/t1b.txt
                fi    
    ...
    if [ $NUMBER_F != $(($NUMBER_TILES*48)) ]; then
        echo "[06:03AM]: MISSING FILES IN DATA DIRECTORY!" >> /home/jline/t1b.txt
